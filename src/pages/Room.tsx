...
export function Room() {
    ...
    async function handleSendQuestion(event: FormEvent) {
        ...
        if (!user) {
            toast.error('Para enviar uma mensagem, você precisa estar autenticado!');
            return;
        }
                    ...
                    <div className="question-list">
                        {questions.map(question => {
                            return (
                                ...
                                >
                                    <button
                                        className={`like-button ${question.likeId ? 'liked' : ''} ${question.isAnswered ? 'disabled' : ''}`}
                                        type="button"
                                        disabled={question.isAnswered || !user}
                                        aria-label="Marcar como gostei"
                                        onClick={async () => await handleLikeQuestion(question.id, question.likeId)}>
...
// Algoritmo de reconciliação - REACT (Necessidade da "Key")